...
from apis.orders.serializers import OrderAddressSerializer, OrderRequestSerializer, OrderListSerializer
...
class OrderRequestListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return OrderRequest.objects.filter(user=self.request.user)\
            .select_related("order_address").order_by("-id")
...
def create_order_request(request: Request):
    ...
    if request_data.get("order_address"):
        new_order_address = get_object_or_404(OrderAddress, pk=request_data.get("order_address"), user=request.user)
    else:
        if not request_data.get("order_address_data", None):
            return Response({"error":"주소를 입력해 주세요"}, status=status.HTTP_400_BAD_REQUEST)

        new_order_address = OrderAddress(
            user=request.user, 
            order_address=request_data.pop("order_address_data")
        )
    ...
    try:
        ...
        return Response(new_order_request_serializer.data, status=status.HTTP_201_CREATED)
